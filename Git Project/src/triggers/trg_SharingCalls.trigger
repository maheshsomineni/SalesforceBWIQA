trigger trg_SharingCalls on Call__c(after insert,after update) {
    ...
    if((Trigger.isAfter && Trigger.isInsert )|| (Trigger.isAfter && Trigger.isUpdate ) ){
      ...
      set<string> lstDepartment = new set<string>();
       ...
       for(Call__c callList : Trigger.new)
        {
       
            lstDepartment.add(callList.Territory__c);
             system.debug('Gaurang : got Territory'+callList.Territory__c); 
        }
      ...
      {
       System.debug('call for loop');
          for(Territory objTerr : lstTerritory)
          {
                 System.debug('Terr for loop'+objTerr);
              if(objCall.Territory__c == objTerr.External_territory_id__c)
              {
              System.debug('if loop');
                  mapTerrExtId.put(objCall.Territory__c, objTerr );
                  System.debug('if loop '+mapTerrExtId);
              }
          }
      }
            ...
            //get the territoryid of of the opportuity 
                ...
                if(objUser.Department != null && objUser.Department!= '' )
                {
                    System.debug('Entered if loop');
                                 ...
                                 if(objUser.Department != job.Territory__c)
                                 {
                                     
                                      Territory objTerritorySec = mapTerrExtId.get(job.Territory__c); 
                                      System.debug('mahesh '+mapTerrExtId);
                                        ...
                                        System.debug('mahesh '+mapTerritoryGroup);
