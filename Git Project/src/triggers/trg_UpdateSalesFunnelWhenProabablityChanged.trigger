trigger trg_UpdateSalesFunnelWhenProabablityChanged on Opportunity (after update) {
...
for(Opportunity objOpp : Trigger.New) 
{
    ...
    if((OldOpp.IsClosed == false) &&( objOpp.IsClosed==false))
    {
        lstId.Add(objOpp.id);
    }
...
List<Opportunity> lstOppTemp = [select Id,Name ,Account.Territory__c, (select Id,Product_type__c,OpportunityId,TotalPrice from OpportunityLineItems ) from Opportunity where id IN : lstId ];
...
List<string> lstOppIds =  new List<string>(); 
        ...
        Opportunity objOpp =  mapOppLineOpp.get(objOppLineItem.id); 
         ...
         if(objOpp != null)
         {
             if( IsParentTerritory(objSale.Territory__c,objOpp.Account.Territory__c)   && objsale.Product_type__c.contains(objOppLineItem .Product_type__c))
             {
                 ...
                 Decimal RiskAdjustedDiff= (objOppLineItem.TotalPrice * objOpp.Probability /100)  - (objOppLineItem.TotalPrice * OldProb /100);
 ...
 {
  EmailforException.notifyDevelopersOf(e,'trg_UpdateSalesFunnelWhenProabablityChanged');
 }
 }
